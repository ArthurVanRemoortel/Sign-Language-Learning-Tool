...
from sl_ai.dataset import preprocess_landmarks, trim_landmark_lists, calculate_landmark_list, \
    pre_process_point_history_center, mirror_landmarks_list
...
def test_auth(request):
    ...
    visualize_gesture(frame_width=frame_width, frame_height=frame_height, coordinates=right_landmarks[ONLY_LANDMARK_ID])
    print("left: ", len(list(filter(lambda p: p != [-1, -1], (left_landmarks[ONLY_LANDMARK_ID])))))
    print("right: ", len(list(filter(lambda p: p != [-1, -1], (right_landmarks[ONLY_LANDMARK_ID])))))
        ...
        for gesture_id, prediction in enumerate(result[0]):
            ...
            if prediction > 10:
                try:
                    ...
                    print(f"Warning: {gesture_id} is present in the model but was probably deleted from the dataset. Retrain the model.")
        pprint(predicted_gestures)
        is_correct = gesture.word.lower() in predicted_gestures or gesture.word in predicted_gestures
